...
import (
	...
	"github.com/khulnasoft/controller-cdktf/gen/random/pet/internal"
)
...
func NewPet(scope constructs.Construct, id *string, config *PetConfig) Pet {
	...
	if err := validateNewPetParameters(scope, id, config); err != nil {
		panic(err)
	}
	j := jsiiProxy_Pet{}
...
func Pet_GenerateConfigForImport(scope constructs.Construct, importToId *string, importFromId *string, provider cdktf.TerraformProvider) cdktf.ImportableResource {
	_init_.Initialize()

	if err := validatePet_GenerateConfigForImportParameters(scope, importToId, importFromId); err != nil {
		panic(err)
	}
...
func (p *jsiiProxy_Pet) AddMoveTarget(moveTarget *string) {
